...
- **LLM 调用层插拔 (LLM Provider Agnostic)**：
    - 核心推理 LLM 通过统一的抽象接口封装，支持**多协议**无缝切换：
        - **OpenAI-Compatible API**：支持 OpenAI 官方接口及其他兼容服务（如小米 MiMo）；
...
- Loader（统一格式与元数据）
	...
	> 
	> **设计优势**：
	> - **零依赖部署**：无需安装 MySQL/PostgreSQL 等数据库服务，`uv pip install` 即可运行
...
> **术语说明**：本节中的"提供者 (Provider)"、"实现 (Implementation)"指的是完成某项功能的**具体技术方案**，而非传统 Web 架构中的"后端服务器"。例如，LLM 提供者可以是远程的 OpenAI-Compatible API（如小米 MiMo），也可以是本地运行的 Ollama；向量存储可以是本地嵌入式的 Chroma，也可以是本地的 BGE 模型。本项目作为本地 MCP Server，通过统一接口对接这些不同的提供者，实现灵活切换。
...
- **统一接口层 (Unified API Abstraction)**：
	- **设计思路**：无论底层使用 OpenAI-Compatible API、DeepSeek 还是本地 Ollama，上层调用代码应保持一致。
	- **关键抽象**：
...
| 提供者类型 | 典型场景 | 配置切换点 |
|---------|---------|-----------|
| **OpenAI-Compatible** | 通用开发、远程 API 调用 | `provider: openai`, `api_key`, `model`, `base_url` |
...
| **BGE (本地 Embedding)** | 本地向量化、无 API 依赖 | `provider: bge`, `model_path` |

- **技术选型建议**：
	- 本项目采用自研的 `BaseLLM` / `BaseEmbedding` 抽象基类，配合工厂模式（`llm_factory.py` / `embedding_factory.py`）实现统一调用接口。已内置 OpenAI-Compatible、Ollama、DeepSeek、BGE 四种 Provider 适配。
	...
	- **Vision LLM 扩展**：针对图像描述生成（Image Captioning）需求，系统扩展了 `BaseVisionLLM` 接口，支持文本+图片的多模态输入。当前实现：
		- **OpenAI-Compatible Vision**（mimo-v2.5）：通过 OpenAI 兼容接口调用多模态模型，支持图片理解。
...
本项目选用 **Chroma** 作为向量数据库。相比 Qdrant、Milvus、Weaviate 等需要 Docker 容器或分布式架构支撑的方案，Chroma 采用嵌入式设计，`uv pip install chromadb` 即可使用，无需额外部署数据库服务，非常适合本地开发与快速原型验证。同时 ChromaStore 适配器（src/libs/vector_store/chroma_store.py），与 Pipeline 无缝集成。
...
- **轻量易部署**：仅需 Python 标准库 + 一个轻量 Web 框架（如 Streamlit），`uv pip install` 即可使用，无需 Docker 或数据库服务。
...
- **组件配置卡片**：读取 `Settings`，展示当前可插拔组件的配置状态：
    - LLM：provider + model（如 `openai / mimo-v2.5-pro`）
...
| 模型 | 提供商 | 特点 | 适用场景 | 推荐指数 |
|-----|--------|------|---------|---------|
| **GPT-4o** | OpenAI | 理解能力强，支持复杂图表解读，英文文档表现优异 | 高质量需求、复杂业务文档、国际化场景 | ⭐⭐⭐⭐⭐ |
...
| 部署环境 | 主选模型 | 备选模型 | 说明 |
|---------|---------|---------|------|
| **本地部署 / Ollama** | Ollama Vision | mimo-v2.5 | 本地离线、隐私敏感场景 |
| **国内部署 / 纯中文场景** | Qwen-VL-Max | mimo-v2.5 | 中文图表理解用 Qwen-VL |
...
|---------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [x] | 2026-05-14 | 目录结构、配置文件、main.py、pyproject.toml 已创建 |
| A2 | 引入 pytest 并建立测试目录约定 | [x] | 2026-05-14 | pytest配置、冒烟测试、样例文档已创建 |
| A3 | 配置加载与校验（Settings） | [x] | 2026-05-14 | Settings数据类、load_settings、validate_settings已实现 |
...
| B6 | Evaluator 抽象接口与工厂 | [x] | 2026-05-14 | BaseEvaluator抽象类、CustomEvaluator(hit_rate/mrr/precision@k)、EvalReport汇总 |
| B7.1 | OpenAI-Compatible LLM 实现 | [x] | 2026-05-14 | OpenAILLM实现，支持自定义base_url |
| B7.2 | Ollama LLM 实现 | [x] | 2026-05-14 | OllamaLLM实现，httpx调用本地Ollama API，mock测试9项 |
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|---------|---------|------|---------|------|
| F1 | TraceContext 增强（finish + 耗时统计 + trace_type） | [x] | 2026-05-18 | 已实现 finish、elapsed_ms、to_dict 并在 Collector 中打通 |
| F2 | 结构化日志 logger（JSON Lines） | [x] | 2026-05-19 | JSONFormatter+get_trace_logger+write_trace，13项测试 |
| F3 | 在 Query 链路打点 | [x] | 2026-05-19 | HybridSearch/Fusion/Reranker trace增强，5项验收测试 |
| F4 | 在 Ingestion 链路打点 | [x] | 2026-05-19 | Pipeline trace增强（load/split/transform/embed/upsert五阶段+elapsed_ms+method），32项测试 |
| F5 | Pipeline 进度回调 (on_progress) | [x] | 2026-05-19 | on_progress回调+5项单元测试 |
...
| G6 | Query 追踪页面 | [x] | 2026-05-19 | query_traces页面+关键词搜索+Dense/Sparse对比+Rerank变化，10项测试 |
