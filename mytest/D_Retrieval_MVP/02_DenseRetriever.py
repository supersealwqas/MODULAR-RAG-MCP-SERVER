...
测试覆盖：
1. 基本检索流程（embed → query → RetrievalResult）
2. top_k 参数传递
3. filters 元数据过滤
4. Trace 记录
5. 空查询边界处理
...
def test_basic_retrieval(retriever: DenseRetriever):
    """测试1: 基本检索流程。"""
    print("\n" + "=" * 60)
    print("测试1: 基本检索流程")
    print("=" * 60)
...
def test_top_k_parameter(retriever: DenseRetriever):
    """测试2: top_k 参数传递。"""
    print("\n" + "=" * 60)
...
def test_filters(retriever: DenseRetriever):
    """测试3: filters 元数据过滤。"""
    print("\n" + "=" * 60)
    print("测试3: filters 元数据过滤")
    print("=" * 60)

    query = "什么是语言模型？"

    # 不带 filters
    results_no_filter = retriever.retrieve(query, top_k=5)
    print(f"  无 filters: {len(results_no_filter)} 条结果")

    # 带 filters（测试空filters不影响结果）
    results_with_filter = retriever.retrieve(query, top_k=5, filters={})
...
def test_trace_recording(retriever: DenseRetriever):
    """测试4: Trace 记录。"""
    print("\n" + "=" * 60)
    print("测试4: Trace 记录")
    print("=" * 60)
    ...
    results = retriever.retrieve(query, top_k=3, trace=trace)

    # 检查 trace 记录
    stages = [s for s in trace.stages if s["name"] == "dense_retrieval"]
    assert len(stages) == 1, f"应记录1个 dense_retrieval 阶段，实际: {len(stages)}"

    stage = stages[0]
    print(f"  记录阶段: {stage['name']}")
    print(f"  结果数量: {stage['result_count']}")
    print(f"  Embed耗时: {stage['embed_ms']}ms")
    print(f"  检索耗时: {stage['query_ms']}ms")
    print(f"  总耗时: {stage['elapsed_ms']}ms")

    assert stage["result_count"] == len(results)
    assert "embed_ms" in stage
    assert "query_ms" in stage
    assert "elapsed_ms" in stage

    print("✅ Trace 记录测试通过")


def test_empty_query(retriever: DenseRetriever):
    """测试5: 空查询边界处理。"""
    print("\n" + "=" * 60)
    print("测试5: 空查询边界处理")
    print("=" * 60)
...
def test_serialization(retriever: DenseRetriever):
    """测试6: RetrievalResult 序列化。"""
    print("\n" + "=" * 60)
    print("测试6: RetrievalResult 序列化")
    print("=" * 60)

    results = retriever.retrieve("什么是语言模型？", top_k=1)
...
def main():
    ...
    # 创建 DenseRetriever（依赖注入，使用真实后端）
    retriever = DenseRetriever(settings)

    # 运行所有测试
    test_basic_retrieval(retriever)
    test_top_k_parameter(retriever)
    test_filters(retriever)
    test_trace_recording(retriever)
    test_empty_query(retriever)
    test_serialization(retriever)
