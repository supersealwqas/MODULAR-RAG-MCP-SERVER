...
Most OpenAI-compatible providers only need `uv pip install openai` (already installed).
Provider-specific SDKs if NOT OpenAI-compatible:

- Qwen: `uv pip install dashscope` (alternative, but OpenAI-compat mode recommended)
- Gemini: `uv pip install google-generativeai` (only if NOT using OpenAI-compat mode)
