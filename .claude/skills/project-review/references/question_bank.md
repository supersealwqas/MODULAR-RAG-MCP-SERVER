...
| 5-04 | Chroma 相对于 Qdrant/Milvus 的核心优势是什么？什么场景考虑换掉它？ | ⭐⭐ | 存储选型 | 嵌入式,uv pip install即用,零部署；规模到分布式/高并发/多节点时考虑换 Qdrant/Milvus |
...
| # | 题目 | 难度 | 考察要点 | 参考答案要点 |
|---|------|------|---------|------------|
| 9-01 | 为什么项目选择 SQLite 而不是 MySQL/PostgreSQL？ | ⭐ | 存储选型 | 零依赖部署(uv pip install)，无需数据库服务；本地优先设计；支持WAL并发；可后续迁移 |
